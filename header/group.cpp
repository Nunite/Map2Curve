			...
			if (Brush.valid)
			{
				...
				for (int f = 0; f<Brush.t_faces; f++)
				{
					...
					if ( Face.FaceIsValid(2,1,1,1) )
					{
						...
						// Im Falle von Grid Path (type=2) sind alle Kanten �blicherweise gleich lang. Das Finden des richtigen Quell-Faces ist daher nicht mit der normalen Methode m�glich.
						// Alle Kanten sind grunds�tzlich gleich lang. Unterschiedlich lange oder Null-Kanten treten nur bei Gap-Brushes auf.
						// Auf die Normale Methode verzichten und bei nicht-Gap-Brushes nur den jeweiligen kleinsten oder gr��ten Pitch suchen, um HSource zu ermitteln.
						// Bei Gap-Brushes dann die normale Methode nutzen.
						
						// Normale Methode: �ber kantenl�nge && (Type!=2 || type==2 && GAP)
					...
					if ( Face.FaceIsValid(2,1,1,1) ) // previously: Face.draw
					{
						...
						for (int bb = b-1; bb>=0; bb--) // start with current brush and decent from here, because others are not relevant
						{
							...
							for (int ff = 0; ff<LBrush.t_faces; ff++)
							{
								...
								{
									// Bei der Suche nach dem letzten g�ltigen hSourceFace sollen wedges mit Edgelen 0 bei Hsrcshift0 �bersprungen werden
